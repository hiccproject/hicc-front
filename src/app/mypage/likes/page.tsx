...
import {
  ...
  type PortfolioReactionListItem,
  type PortfolioReactionListRawResponse,
  type PortfolioReactionListResponse,
} from "@/lib/api/cards";
...
type PortfolioListItem = PortfolioReactionListItem;
type PortfolioListResponse = PortfolioReactionListResponse;
...
function normalizeListResponse(raw: PortfolioReactionListRawResponse): PortfolioListResponse {
  const payload = (
  ...
  const fetchPage = useCallback(
    async (pageNumber: number) => {
      ...
      try {
        ...
        const raw = await getLikedPortfolios(params);
        ...
        setTotalPages(
          typeof data?.totalPages === "number" && data.totalPages > 0 ? data.totalPages : null
        );
      } catch (fetchError) {
    ...
    <div className={styles.page}>
      ...
      <main className={styles.container}>
        ...
        {!loading && items.length === 0 && !error ? (
          <p className={styles.empty}>좋아요한 명함이 없습니다.</p>
        ) : null}
